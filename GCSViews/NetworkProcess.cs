...
namespace MissionPlanner
{
    ...
    public class NetworkProcess
    {
        ...
        public void StartListening()
        {
            ...
            IPAddress ipAddress = ipHostInfo.AddressList[5];    // swan home: [5], swan mac(win): [2]
        ...
        public void ReadCallback(IAsyncResult ar)
        {
            ...
            if (bytesRead > 0)
            {
                ...
                if (content.IndexOf("<EOF>") > -1)
                {
                    ...
                    {
                        MakeReturnWP(User);
                        MainV2.instance.FlightPlanner.DAEMON_setRTLLand(this, handler);
                    ...
                    else
                    {
                        ...
                        if (totalData[User].Count > 2)  // 수신 이전꺼 사용: 머리위ㅇ로 뜨는거 최소화
                        {
                            MainV2.instance.FlightPlanner.DAEMON_loadwpfile(this, null);
                            MainV2.instance.FlightPlanner.DAEMON_writeLocation(this, handler);
        ...
        /// <summary>
        /// 응답 ACK 보내기. ack: 1로 고정
        /// </summary>
        public void SendACK(Socket handler)
        {
            AckType obj = new AckType();
            obj.ack = "1";

            MemoryStream stream = new MemoryStream();
            DataContractJsonSerializer ser = new DataContractJsonSerializer(typeof(AckType));
            ser.WriteObject(stream, obj);
            stream.Position = 0;
            StreamReader sr = new StreamReader(stream);
            
            
            Send(handler, sr.ReadToEnd()+Environment.NewLine);
        ...
        private bool SaveData(string content)
        {
            ...
            if (dList.Count > 2)    // home 위치를 처음 수신한 곳으로 고정 (2: 
        ...
        private void SaveLocToWPFile(string _id)
        {
            ...
            // 수신한 위치의 바로 직전 위치로 .. 머리위로 뜨기 방지
            DataType home = list[0];
            DataType current = list[list.Count - 2];
