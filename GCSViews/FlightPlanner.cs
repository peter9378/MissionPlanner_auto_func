...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public void DAEMON_loadwpfile(object sender, EventArgs e)
        {
            NetworkProcess _sender;
            string padding, curUser;

            if (sender.GetType().Equals(typeof(NetworkProcess)))
            {
                _sender = (NetworkProcess)sender;
                curUser = _sender.User;
            }
            else return;    // temporary fix
            
            // false: 0 true: 1 >> 작업 후 tick을 flip했으므로 반대로 읽어야함 (1, 0)
            if (_sender.tick)   // true -> 0 read
            {
                padding = "_0";
            }
            else
            ...
            string file = _sender.User + padding + ".waypoints";

            // Mission Planner original codes
            //string file = "plan.waypoints";
            string line = "";
            using (var fs = File.OpenText(file))
            {
                line = fs.ReadLine();
        ...
        public void DAEMON_writeLocation(object sender, Socket handler)
        {
            NetworkProcess _sender;
            string curUser;
            ...
            */
            ...
            //{
            //  MainV2.comPort.setMode("Loiter"); // 로이터 후 오토 해도 안돌아오네;;
            MainV2.comPort.setMode("Auto");
            //}

            _sender.SendACK(handler);
            //MainMap.Focus();
        }

        public void DAEMON_setRTLLand(object sender, Socket handler)
        {
            NetworkProcess _sender;
            ...
            /////// Write WP Mission
            DAEMON_writeLocation(_sender, handler);
        }
        ...
        private void testButton_Click(object sender, EventArgs e)
        {
            ...
            //            var armed = MainV2.comPort.MAV.cs.armed;     // property name as armed

            //            MainV2.comPort.doARM(!armed);
            //            //MainV2.comPort.doCommand(MAVLink.MAV_CMD.TAKEOFF, 1, 0, 0, 0, 0, 0, 0);
            //            //System.Threading.Thread.Sleep(10000);

            //            // takeoff
            //            //DAEMON_writeLocation(sender, e);    // set WPs
            ...
            //            //MainV2.comPort.SendRCOverride(MainV2.comPort.MAV.sysid, MainV2.comPort.MAV.compid, 1500, 1500, 1080, 1500, 1150, 992, 993, 1504);
            //            //MainV2.comPort.setMode("Auto");
            ...
            //            //MainV2.comPort.doCommand(MAVLink.MAV_CMD.TAKEOFF, 0,0,0,0,0,0,0 );
            //            //Thread.Sleep(1500)
            //            //MainV2.comPort.setMode("Auto");
        ...
        // Networking Threading Object
        public Thread t1 = new Thread(new ThreadStart(new NetworkProcess().StartListening));
